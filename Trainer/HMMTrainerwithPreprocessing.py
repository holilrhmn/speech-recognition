...
shoes = "../data/voicebank/test/shoes1.wav"
textbox =  "../data/voicebank/test/textbook1.wav"
get = "../data/voicebank/test/map1.wav"
...
shoes = np.concatenate((mfcc_feat, d_mfcc_feat), axis=1)
print ("d_mfcc_feat : " , d_mfcc_feat)




(rate, sig) = wav.read(textbox)
# print "Read : " , ses_yol + word + "/" + word + str ( i / 10 ) + str ( i %10 ) +".wav"
mfcc_feat = mfcc(sig, rate, nfft=1536)
d_mfcc_feat = delta(mfcc_feat, 2)

textbox = np.concatenate((mfcc_feat, d_mfcc_feat), axis=1)
...
for word in word_list:
    ...
    for i in file_list:
        ...
        concatenated = np.concatenate((mfcc_feat, d_mfcc_feat), axis=1).tolist()
...
pickle.dump ( hmm_list , open ( "TrainedHmmsPreprocessing.hmm" , "wb"))
pickle.dump ( scalers, open("ScalersPreprocessing.scl", "wb"))
print ("HMM models have been saved....\n\n")

print(test(scalers["textbook"].transform(textbox), hmm_list))
print(test(scalers["shoes"].transform(shoes), hmm_list))
